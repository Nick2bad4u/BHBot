...
LAYOUT_MAPPING = {
    ...
    "press_start": [
        'Нажми "Старт" чтобы начать работу бота',
        'Нажми "Стоп" чтобы завершить работу бота',
    ],
    "toggle": ["Старт", "Стоп"],
    "delayed_stop": ["Отложенный Стоп", "Отменить Стоп"],
    "settings": "Настройки",
...
TOOLTIPS = {
    # Основное окно
    "delayed_stop": [
        "Выключить бота после завершения текущей игры или перед началом следующей",
...
MESSAGES = {
    ...
    "no_steam_exe": "Файл запуска клиента Steam не найден. Попробуйте запустить Steam и включить бота заново",
    "delayed_stop": "Бот автоматически выключится после завершения текущей игры или перед началом следующей",
    "cancel_stop": "Все отложенные выключения отменены",
    "menu_pixels_error": "Ошибка при получении пикселей для определения состояния. Не волнуйтесь, это просто значит что будут использованы значения по умолчанию:\n%s",
    "took_screenshot": "Снимок экрана сохранен в %s",
    "bh_not_running": "Игра не запущена",
}
